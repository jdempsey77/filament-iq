...
- [x] SNAPSHOT_IMPLAUSIBLE false positive for non-RFID slots — _is_rfid_slot() guard added to implausibility check. Non-RFID Spoolman fallback of 0.0 no longer excluded. Confirmed data loss: spool 76, ~400g. (v1.6.3, Bug 16)
...
- [ ] Spool 76 consumption correction — ~400g unrecorded across multiple prints (2026-03-25) due to SNAPSHOT_IMPLAUSIBLE false positive (Bug 16, now fixed). Spoolman currently shows 600g remaining (manual estimate). Actual remaining unknown — requires cross-referencing print history with slicer estimates.
...
- [x] 3MF fetch Phase 3 optimization — superseded by v1.7.0 cache-first path. FTPS 530 errors eliminated via ha-bambulab cache. (v1.7.0)
...
- [x] RunoutTracker zero-write — when a spool physically runs out mid-print, `input_boolean.ams_slot_N_ran_out` is set on by RunoutTracker but Spoolman's `remaining_weight` may still show a non-zero estimate. Fixed in `AMSPrintUsageSync`: (1) `_apply_runout_zero_overrides` called after `_execute_writes` — if ran_out boolean is on and post-write remaining > 0, issues `PATCH remaining_weight=0` and logs `RUNOUT_ZERO_OVERRIDE`; (2) `_reconcile_rfid_weight_slot` skips any slot where ran_out boolean is on, logging `RECONCILE_RUNOUT_SKIP` (prevents reconciler overwriting the zero ~60s after print end). (v1.10.1)
- [ ] RunoutTracker graceful startup retry — currently hard-fails with
  PROBE_MISSING if input_boolean helpers aren't registered in HA yet
  (e.g. after a cold reboot). Add retry-with-backoff so the app
  self-heals once helpers become available instead of requiring a
  manual AppDaemon restart.
- [x] 3MF cache read optimization — ha-bambulab cache-first (slice_info.config), FTPS fallback. Eliminates 530 errors for non-RFID slots. (v1.7.0)
