...
- [ ] Runout split finishing slot write lost on rehydrated prints — `RUNOUT_SPLIT_DETECTED` correctly computes finishing_share (e.g. 149.38g) but `USAGE_NO_EVIDENCE slot=N reason=BELOW_MIN: 0.00g` fires for the finishing slot. Root cause: active_slots narrowing excludes the finishing slot on rehydrated prints before the runout split result can be applied. Confirmed data loss: 149.38g (2026-03-25, spool_id=72, slot=3, job=0.2mm_layer_2_walls_15%_infill_1774386264). Same class of bug as 3MF_UNMATCHED rehydration fix (v1.5.2). Needs ANALYZE pass first.
