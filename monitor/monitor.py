...
PRINTER_PREFIX = "p1s_01p00c5a3101668"
PRINT_STATUS_ENTITY = f"sensor.{PRINTER_PREFIX}_print_status"
TASK_NAME_ENTITY = f"sensor.{PRINTER_PREFIX}_task_name"
PRINT_PROGRESS_ENTITY = f"sensor.{PRINTER_PREFIX}_print_progress"
ACTIVE_TRAY_ENTITY = f"sensor.{PRINTER_PREFIX}_active_tray"

TRAY_ENTITIES = {
    1: f"sensor.{PRINTER_PREFIX}_ams_1_tray_1",
    2: f"sensor.{PRINTER_PREFIX}_ams_1_tray_2",
    3: f"sensor.{PRINTER_PREFIX}_ams_1_tray_3",
    4: f"sensor.{PRINTER_PREFIX}_ams_1_tray_4",
    5: f"sensor.{PRINTER_PREFIX}_ams_128_tray_1",
    6: f"sensor.{PRINTER_PREFIX}_ams_129_tray_1",
}

SLOT_TO_SPOOL_ENTITIES = {
    s: f"input_text.ams_slot_{s}_spool_id" for s in range(1, 7)
}
...
class Config:
    ...
    def __init__(self):
        ...
        self.ha_token = secrets.get("HA_TOKEN", "")
...
def _snapshot_spoolman_weights(cfg: Config) -> dict:
    ...
    slot_bindings = {}
    for slot, entity in SLOT_TO_SPOOL_ENTITIES.items():
...
class PrintLifecycleMonitor:
    ...
    def _get_gcode_state(self) -> str:
        state = _ha_get_state(self.cfg, PRINT_STATUS_ENTITY)
        if state:
            return str(state.get("state", "")).strip().lower()
        return ""

    def _get_job_name(self) -> str:
        state = _ha_get_state(self.cfg, TASK_NAME_ENTITY)
        if state:
            return str(state.get("state", "")).strip()
        return ""

    def _get_progress(self) -> int:
        state = _ha_get_state(self.cfg, PRINT_PROGRESS_ENTITY)
        if state:
            try:
    ...
    def _get_active_tray(self) -> int | None:
        state = _ha_get_state(self.cfg, ACTIVE_TRAY_ENTITY)
        if state:
            try:
...
def main() -> None:
    ...
    cfg = Config()
    log.info("Config loaded — HA: %s, Spoolman: %s, Artifacts: %s", cfg.ha_url, cfg.spoolman_url, cfg.artifact_root)
